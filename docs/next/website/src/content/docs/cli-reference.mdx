...
```bash
herdr service add <label> <url> [--workspace <id>] [--source <text>] [--json]
herdr service list [--workspace <id>] [--json]
herdr service remove (<id> | --label <label>) [--workspace <id>] [--json]
```

Services register long-running development servers and listening ports with a workspace. Each service has a label, URL, and optional source attribution. Use `service add` to register a new service or update an existing one by label (upsert behaviour).

URL forms accepted: `http://…`, `https://…`, `host:port`, `:3000` (→ `http://localhost:3000`).

When called from inside a Herdr pane, `service add` uses the calling pane's workspace by default. Pass `--workspace <id>` to override. Outside a pane, `--workspace <id>` is required.

The `--source` parameter is optional and records who registered the service (e.g. `"cli"`, `"plugin:example"`). It appears in the service list.

Use `--json` with any command for deterministic automation in scripts.
