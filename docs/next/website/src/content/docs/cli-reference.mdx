...
Supported commands are `workspace`, `worktree`, `service`, `tab`, `pane`, `notification`, `agent` (except `attach` and local `explain --file`), `api snapshot`, `status server`, and API-backed plugin commands (`link`, `unlink`, `enable`, `disable`, `list`, `action`, `log`, `pane`). Server commands support `stop`, `reload-config`, `agent-manifests`, and `reload-agent-manifests`. Local installation/configuration commands, plugin installation, session management, and interactive terminal attachment are not forwarded.
...
```bash
herdr service add <label> <url> [--workspace <workspace_id>] [--source TEXT]
herdr service list [--workspace <workspace_id>]
herdr service remove (<service_id> | --label <label>) [--workspace <workspace_id>]
```

Services are long-running development servers and listening ports registered against a workspace. The sidebar shows a `▸N` chip on a workspace that has services; click it to expand one row per service with a liveness dot, its label, and its URL, and click a row to open the URL in your browser.

`service add` is an upsert keyed by label: registering an existing label again updates its URL (and resets its liveness) and returns the same `service_id`. Agents should re-run it whenever they restart a server. Accepted URL forms are `http://…`, `https://…`, `host:port`, and `:port`, which becomes `http://localhost:port`. Anything without a resolvable host and port is rejected with `invalid_url`.

Inside a Herdr pane the workspace is taken from `HERDR_WORKSPACE_ID`, or from the calling pane via `HERDR_PANE_ID`, so agents can register a service without knowing any IDs. Pass `--workspace <workspace_id>` to target another workspace; outside a pane it is required. `--source TEXT` records who registered the service (default `cli`) and is returned by `service list`.

The server probes every registered service with a short TCP connect roughly every five seconds. `service list` reports `liveness` as `unknown`, `up`, or `down`. Services persist across server restarts and restore as `unknown` until the first probe completes.
