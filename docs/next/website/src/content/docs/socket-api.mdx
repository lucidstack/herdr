...
Service methods register long-running development servers and listening ports with a workspace. Each service has a label, URL, optional source attribution, and a liveness status determined by periodic TCP connect probes.

`service.add` registers a new service or updates an existing one by label (upsert behaviour). Accepted URL forms: `http://…`, `https://…`, `host:port`, `localhost:3000`, `:3000` (→ `http://localhost:3000`).

Attribution happens automatically from the calling context: when invoked from a CLI pane via `herdr service add` inside Herdr, it resolves the workspace from the injected `HERDR_WORKSPACE_ID` environment variable (or falls back to `HERDR_PANE_ID` → the pane's workspace). Outside a pane, `workspace_id` is required.

Register a service:

```json
{"id":"req_1","method":"service.add","params":{"workspace_id":"w1","label":"Rails server","url":":3000","source":"cli"}}
```

List services for a workspace:

```json
{"id":"req_2","method":"service.list","params":{"workspace_id":"w1"}}
```

Remove a service by ID or label:

```json
{"id":"req_3","method":"service.remove","params":{"workspace_id":"w1","id":1}}
```

Service responses include the service ID, label, URL, source, and liveness status (one of `unknown`, `up`, `down`). Liveness is unknown until the first probe succeeds or fails; it is not persisted across server restarts. Services persist in the session snapshot.

`service.add` errors with `attribution_required` when workspace cannot be resolved, `workspace_not_found` when the specified workspace does not exist, `pane_not_found` when the caller references an unknown pane, or `invalid_url` when the URL is malformed.

`service.remove` errors with `service_not_found` when neither the ID nor label matches an existing service.
...
Service event subscriptions include `services_changed`, which is emitted when the service list for a workspace changes (service added, updated, or removed). `services_changed` includes the `workspace_id` of the affected workspace.
