...
| Service | `service.add`, `service.list`, `service.remove` |
...
Service methods register long-running development servers and listening ports against a workspace. Each service has a workspace-scoped numeric `id`, a `label`, a normalised `url`, a free-text `source`, and a server-computed `liveness` of `unknown`, `up`, or `down`.

`service.add` and `service.remove` resolve their workspace the same way: an explicit `workspace_id` wins, otherwise `pane_id` selects the workspace that owns that pane, otherwise the request fails with `attribution_required`. Processes inside a Herdr pane can pass the injected `HERDR_WORKSPACE_ID` or `HERDR_PANE_ID` values directly.

`service.add` is an upsert keyed by `label`: an existing label keeps its `id`, takes the new `url` and `source`, and resets its liveness to `unknown` when the URL changed. `url` accepts `http://…`, `https://…`, `host:port`, and `:port` (normalised to `http://localhost:port`); anything without a resolvable host and port fails with `invalid_url`. `source` defaults to `cli`.

```json
{"id":"req_1","method":"service.add","params":{"pane_id":"w1:p1","label":"Rails server","url":":3000","source":"agent"}}
{"id":"req_1","result":{"type":"service_added","service_id":1,"workspace_id":"w1"}}
```

`service.list` takes an optional `workspace_id`; omit it to list every workspace's services.

```json
{"id":"req_2","method":"service.list","params":{"workspace_id":"w1"}}
{"id":"req_2","result":{"type":"service_list","services":[{"workspace_id":"w1","id":1,"label":"Rails server","url":"http://localhost:3000","source":"agent","liveness":"up"}]}}
```

`service.remove` takes exactly one of `id` or `label` plus the attribution fields above, and returns `ok`. It fails with `service_selector_required` when both are missing and `service_not_found` when nothing matches.

```json
{"id":"req_3","method":"service.remove","params":{"workspace_id":"w1","label":"Rails server"}}
```

Liveness comes from a server-side probe that runs roughly every five seconds while at least one service exists, attempting a short TCP connect to each service's host and port; the probe never runs while a previous one is still in flight. Services persist in the session snapshot; liveness does not, so restored services report `unknown` until the first probe completes.
...
The `services.changed` subscription delivers `services_changed` whenever a workspace's service list changes through `service.add` or `service.remove`. Each event includes the affected `workspace_id` and the full updated `services` list in the same shape as `service.list`. Liveness probe results do not emit events; poll `service.list` or read the shell snapshot for liveness.
