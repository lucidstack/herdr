...
Links you open over SSH, from a pane or from a work item's **Open on GitHub**, would otherwise open in the server's browser. Herdr shows them instead: tap the link, or press <kbd>Enter</kbd> to copy it to your phone's clipboard. Copying works when your terminal accepts clipboard writes (OSC 52); mosh needs version 1.4 or later on both ends. Set `ui.open_links` to change this.
