...
A service registers a long-running development server or listening port with a workspace. Services are shown in the sidebar under the workspace row when expanded, displaying the service label, URL, and a liveness indicator (dot: unknown, up, or down).

Click a service row to open its URL in your system browser. Liveness is determined by periodic TCP connect probes; it starts as unknown and updates as probes succeed or fail.
...
Services belong to one workspace, are named by label (re-adding updates the URL), and persist across server restarts but restore with unknown liveness until the first probe.
