...
A service is a long-running development server or listening port registered against a workspace, such as a Rails server on `:3000`. Agents and scripts register one from inside a pane:

```bash
herdr service add "Rails server" :3000
```

A workspace with services shows a `▸N` chip in the sidebar. Click it to expand one row per service with a liveness dot (grey unknown, green up, red down), the label, and the URL; click a row to open the URL in your browser. Re-adding a label updates its URL. Services persist across server restarts and restore as unknown until the server's periodic TCP probe reaches them. See [CLI reference](/docs/cli-reference/#services) and the `service.*` methods in [Socket API](/docs/socket-api/).
